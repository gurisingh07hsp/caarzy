...
const PicturesPage = () => {
  ...
  const [selectedtab, setSelectedTab] = useState<
    "description" | "overview" | "images" | "pros&cons" | "reviews"
  >("description");
  const getCars = async () => {
    try {
      const response = await axios.get(
        `/api/managemodels/${brand?.toString().replace(/-/g, " ")}/${name?.toString().replace(/-/g, " ")}`,
      );
  ...
  const navigate = useCallback(
    (dir: 1 | -1) => {
      ...
      setLightboxIdx(
        (lightboxIdx + dir + car.interiorImages.length) %
          car.interiorImages.length,
      );
    },
    [lightboxIdx, car?.interiorImages?.length],
  );

  // const [featured, ...rest] = car && car.interiorImages;

  if (loading) {
    return <CarLoadingComponent />;
  }
  return (
    <div className="mb-4 max-w-7xl mx-auto">
      <div>
        <div className="mb-6 rounded-3xl flex lg:flex-row flex-col gap-2 bg-gray-100 p-4">
          <div>
          ...
          <div>
            <p className="text-slate-600 mb-2 px-4">
              {capitalizeString(car.brand)} •{" "}
              {car.bodyType == "suv" ? "SUV" : capitalizeString(car.bodyType)}
            </p>
            <h1 className="text-3xl font-bold text-slate-900 mb-2 px-4">
              {capitalizeString(car.modelName)}
            </h1>
                    ...
                    <div>
                      {(() => {
                        const min = Math.min(
                          ...car.variant.map(
                            (v: any) =>
                              v?.engineAndTransmission?.Range.split(" ")[0] ||
                              0,
                          ),
                        );
                        const max = Math.max(
                    ...
                    <div>
                      {(() => {
                        const min = Math.min(
                          ...car.variant.map(
                            (v: any) =>
                              v?.fuelAndPerformance?.petrolMileageARAI?.split(
                                " ",
                              )[0] || 0,
                          ),
                        );
              ...
              <div className="rounded-2xl overflow-hidden">
                <div className="grid gap-0">
                  <div className="p-4">
                    {(() => {
                      const min = Math.min(
                        ...car?.variant.map((v: any) => v.price),
                      );
                      const max = Math.max(
                        ...car?.variant.map((v: any) => v.price),
                      );
                      return (
                        <p className="text-2xl font-bold main-text-color">
                          ₹{PriceFormatter(min)} - {PriceFormatter(max)}
                        </p>
                      );
                    })()}
        ...
        <div className="lg:p-6 p-3 border border-stone-200 rounded-2xl mt-4 bg-stone-50">
          {/* Header */}
          <div className="flex items-end justify-between mb-5 flex-wrap gap-2">
            ...
            <span className="text-xs tracking-widest text-stone-400 uppercase">
              {car.exteriorImages.length} Photos
            </span>
          </div>

          {/* Divider */}
          <div className="h-px bg-gradient-to-r from-stone-300 to-transparent mb-5" />
          ...
          <div className="grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-3">
            {car.exteriorImages.map((src: string, index: number) => (
              <div
                key={index}
                className="group relative overflow-hidden rounded-xl cursor-pointer bg-stone-200 aspect-video lg:aspect-square"
              >
                <img
                  src={src}
                  alt={car.modelName}
                ...
                {/* Zoom icon */}
                <div className="absolute top-2 right-2 w-7 h-7 rounded-full bg-white/10 border border-white/25 backdrop-blur-sm flex items-center justify-center text-white text-xs opacity-0 scale-75 group-hover:opacity-100 group-hover:scale-100 transition-all duration-300">
                  ⤢
                </div>
              </div>
            ))}
        ...
        <div className="lg:p-6 p-3 border border-stone-200 rounded-2xl mt-4 bg-stone-50">
          ...
          <div className="flex items-end justify-between mb-5 flex-wrap gap-2">
            ...
            <span className="text-xs tracking-widest text-stone-400 uppercase">
              {car.interiorImages.length} Photos
            </span>
          </div>
