    ...
    <section id='brands' className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 md:gap-6">
        {brands.slice(0,num).map((brand,index) => (
          <div onClick={()=>router.push(`/brand/${brand.name.toLowerCase().replace(/\s+/, '-')}`)} key={index} className="bg-white hover:shadow-md rounded-xl border cursor-pointer border-gray-100 shadow-sm md:p-4 text-center">
            <img src={brand.logo} alt={brand.name} className="w-28 h-14 md:h-20 mx-auto mb-3 object-contain" />
