...
export function UpcomingCars({ cars}: UpcomingCarsProps) {
    ...
    const upcomingCars = cars.filter((car) => car.category === 'Upcoming Cars');
    const list = upcomingCars;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {list.map((car,index) => (
          ...
          <div key={car.modelName}
            ...
            className="group bg-card border border-border rounded-2xl overflow-hidden hover:shadow-lg hover:border-red-200 transition-all cursor-pointer hover:-translate-y-1">
            {/* Image */}
            <div className="relative aspect-video overflow-hidden bg-secondary">
              <img src={car.images[0]} alt={car.modelName} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
              {/* Year badge */}
              <div className="absolute bottom-2 left-2 px-2 py-0.5 rounded-lg text-xs font-black main-text-color" style={{ background: "hsl(var(--accent))" }}>
                 Expected Date {car && car.launchDate && new Date(car.launchDate).toLocaleDateString('en-IN', { day:'numeric', month: 'short', year: 'numeric' })}
              </div>
            </div>
            {/* Info */}
            <div className="p-4">
              <p className="text-xs text-muted-foreground">{car.brand.charAt(0).toUpperCase() + car.brand.slice(1)} · {car.bodyType === 'suv' ? 'SUV' : car.bodyType.charAt(0).toUpperCase() + car.bodyType.slice(1)}</p>
              <h3 className="font-bold text-foreground mt-0.5 mb-2 group-hover:text-red-600 transition-colors">{capitalizeString(car.modelName)}</h3>
              <div className="flex items-center justify-between">
                  <div>
                {car.variant.length > 0 && (
                  <div>
                    <p className="text-[10px] text-muted-foreground">Ex-Showroom</p>
                    <p className="text-sm font-black" style={{ color: "hsl(var(--price-color))" }}>₹{PriceFormatter(car?.variant[0]?.price)}</p>
                  </div>
                )}
                </div>
                <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center group-hover:bg-red-600 transition-colors">
