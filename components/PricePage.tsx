  ...
  return (
     <div className="lg:max-w-7xl mx-auto lg:px-4 px-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div>
            <div className="relative rounded-2xl overflow-hidden bg-secondary aspect-[4/3]">
              ...
              {/* <div className="absolute top-3 right-3 flex gap-1.5">
                ...
                <button className="w-8 h-8 rounded-full bg-card/80 backdrop-blur flex items-center justify-center shadow-sm hover:text-primary transition-colors"><Share2 className="w-4 h-4" /></button>
              </div> */}
            </div>
        ...
        <div className='mt-4'>
        <p className='lg:text-2xl text-lg mb-2 font-medium'>{capitalizeString(car?.modelName)} {capitalizeString(variant as string).replace(/-/g, ' ')}</p>
        <div>{`The on-road price of the ${capitalizeString(car?.modelName)} in ${variant?.toString().split('-').pop()} begins from ₹${PriceFormatter(car?.variant[0]?.price)}. The highest-priced variant 
            goes up to ₹${PriceFormatter(car?.variant[car?.variant.length - 1]?.price)}. ${car.variant.filter((v: any) => v.engineAndTransmission.transmissionType == 'Automatic').length > 0 ? `The automatic variants are priced between ₹${PriceFormatter(car?.variant.filter((v: any) => v.engineAndTransmission.transmissionType == 'Automatic')[0]?.price)} and ₹${PriceFormatter(car?.variant.filter((v: any) => v.engineAndTransmission.transmissionType == 'Automatic')[car?.variant.filter((v: any) => v.engineAndTransmission.transmissionType == 'Automatic').length - 1]?.price)},` : ''} ${car.variant.filter((v: any) => v.fuelAndPerformance?.fuelType == 'CNG').length > 0 ? `while the CNG versions range from ₹${PriceFormatter(car?.variant.filter((v: any) => v.fuelAndPerformance?.fuelType == 'CNG')[0]?.price)} to ₹${PriceFormatter(car?.variant.filter((v: any) => v.fuelAndPerformance?.fuelType == 'CNG')[car?.variant.filter((v: any) => v.fuelAndPerformance?.fuelType == 'CNG').length - 1]?.price)}, making it suitable for different driving needs and 
            budgets.` : ''} Pricing may vary depending on registration charges, insurance, and optional accessories 
            available in ${variant?.toString().split('-').pop()}.`}</div>
      </div>
